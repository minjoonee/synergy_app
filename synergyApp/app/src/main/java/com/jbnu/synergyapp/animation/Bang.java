package com.jbnu.synergyapp.animation;
...
public class Bang extends View {
        ...
        private BangListener mListener;
        private int centerY;
        private int centerX;

        // 다음 View 를 줄입니다.
        // 색상 그라데이션(p1)을 사용하는 동안 원의 반지름을 작게 커팅합니다.
        // 반경이 MAX_RADIUS에 도달하면 열린 원을 그리기 시작하고,
        // 자유 원의 반경이 커지고 펜의 폭은 MAX_RADIUS 에서 작아집니다.
        // 열린 원의 반지름이 MAX_RADIUS - RING_WICTH (p2)에 도달하면 반지 형태가 되고,
        // 원에 DOT_NUMBER 개 만큼의 작은 원이 고르게 분포됩니다.
        // 원이 점점 커지면서 점진적인 고리가 사라지고 동시에 작은 원이 바깥으로 퍼지고
        // 확산 과정에서 작은 원의 반지름이 줄어들고 색상이 변하면서
        // 동시에 글씨가 점점 커집니다.(p3)
        ...
        public Bang(Context context, AttributeSet attrs) {
                super(context, attrs);
                init(attrs, 0);
        }

        public Bang(Context context, AttributeSet attrs, int defStyleAttr) {
                super(context, attrs, defStyleAttr);
                init(attrs, defStyleAttr);
        }

        @TargetApi(Build.VERSION_CODES.LOLLIPOP)
        public Bang(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
                super(context, attrs, defStyleAttr, defStyleRes);
                init(attrs, defStyleAttr);
        }

        public static Bang attach2Window(Activity activity) {
                ViewGroup rootView = (ViewGroup) activity.findViewById(Window.ID_ANDROID_CONTENT);
                Bang smallBang = new Bang(activity);
         ...
         * @param listener
         */
        public void setmListener(BangListener listener) {
        ...
        public void bang(final View view, BangListener listener) {
                bang(view, -1, listener);
        }

        public void bang(final View view, float radius, BangListener listener) {
