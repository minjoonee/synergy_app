...
//session_start();
  ...
  $table="freeboard_tb";
   ...
   //select *from freeboard_tb where title like "%content%" or content like "%content%";
   $query="select *from freeboard_tb where title like '%$data%' or content like '%$data%' order by num desc;";
...
<ul class="search">
  <li class="search_icon_li"><form class="search_icon" action="m_freeboard_search.php" method="post">
	...
	<table>
	<tr><th colspan='2'><a href='m_freeboard_view.php?num=$array[num]'>$array[title]</a></th></tr> <tr><td>$array[date]</td><td>$array[id]</td></tr>   	</table>
	</div>
	");
	}
 ?>
